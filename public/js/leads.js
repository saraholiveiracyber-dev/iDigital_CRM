...
function obterNomeOrigem(origem) {
    ...
    return nomes[chave] || String(origem || "Outro");
}
...
function normalizarEbook(item) {
    ...
    const statusOriginalNormalizado =
    ...
    if (
        item.paid_at &&
        !["fechado", "cancelado"].includes(status)
    ) {
        status = "pendente";
    }
    ...
    if (
        [
            ...
            "pagamento_aprovado",
            "paid"
        ].includes(statusOriginalNormalizado)
    ) {
        status = "pendente";
    }
    ...
    return {
        ...
        nome: String(
            campo(item, ["nome"], "Cliente do e-book")
        ),
        ...
        origem: "kiwify",
        origemLabel: "Kiwify / E-book",

        observacoes: [
            item.descricao
                ? `Briefing: ${item.descricao}`
                : "",
        ...
        kiwify_order_id: item.kiwify_order_id || "",
...
function normalizarLead(item) {
    ...
    return {
        ...
        origem: String(
            campo(item, ["origem"], "outro")
        ),

        origemLabel: obterNomeOrigem(
            campo(item, ["origem"], "outro")
        ),
...
async function carregarOportunidades() {
    ...
    try {
        orcamentos = await buscarOrcamentos();
    } catch (error) {
        console.error(
    ...
    try {
        leads = await buscarLeadsManuais();
    } catch (error) {
        console.error(
            "[CRM] Erro em leads:",
            error
    ...
    oportunidades = [
        ...orcamentos.map(normalizarOrcamento),
        ...ebooks.map(normalizarEbook),
        ...leads.map(normalizarLead)
    ];
...
function atualizarEstatisticas() {
    ...
    const orcamentos =
        oportunidades.filter(
            item => item.tipo === "orcamento"
        ).length;

    const ebooks =
        oportunidades.filter(
            item => item.tipo === "ebook"
        ).length;
...
function renderizarTabela(dados = oportunidades) {
    ...
    dados.forEach(
        oportunidade => {
            ...
            const nome =
                escapar(oportunidade.nome);

            const email =
                escapar(oportunidade.email);

            const contato =
                escapar(oportunidade.contato);

            const servico =
                escapar(oportunidade.servico);

            const origem =
                escapar(oportunidade.origemLabel);
            ...
            const id =
                escapar(oportunidade.id);

            const origemId =
                escapar(oportunidade.origemId);
...
function abrirWhatsApp(oportunidade) {
    ...
    const mensagem =
        `Olá, ${
            oportunidade.nome || ""
        }! Tudo bem? Estou entrando em contato sobre seu ${
            oportunidade.tipo === "ebook"
                ? "pedido de criação de e-book"
...
function abrirPagamento(oportunidade) {

    if (!oportunidade?.payment_url) {
        return;
    }
...
function limparFormulario() {
    ...
    if (leadEtapa) {
        leadEtapa.value = "novo";
    }

    if (leadOrigem) {
        leadOrigem.value = "outro";
    }
...
function editarLead(id) {
    ...
    if (leadId) {
        leadId.value = lead.origemId;
    }

    if (leadNome) {
        leadNome.value = lead.nome || "";
    }
...
async function salvarLead(event) {
    ...
    const payload = {
        ...
        contato:
            leadContato?.value.trim() || null,

        email:
            leadEmail?.value.trim() || null,
        ...
        servico:
            leadServico?.value.trim() || null,

        observacoes:
            leadObs?.value.trim() || null
    };
